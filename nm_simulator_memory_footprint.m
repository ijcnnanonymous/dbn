...
DspRes = 20; slot_count = zeros(1,10); bandwidth = zeros(1,10); datawidth = 32;
Ft=zeros(400,400*DspRes); view=zeros(516,1000*DspRes);

cp=1; ncp=1; cp_cnt = 0; dp_i=0; d_i=0; net_sum = 0; % initialize variables
sbase = 0; s_en = 0; nrsel = 0; nren = 0; done = 0; err = 0;
cmap = lines(9); cmap(1,:) = 1;
...
for ck=1:1E9     % clock cycles
  cPh = OP(cp,1); nPh = OP(ncp,1); ck1 = mod(ck,400*DspRes)+1; % shortcuts
...
%   [ck cp ncp sbase delaycnt]
  ...
  for k=1:64    % loop(k) computes SNU(k)
    ...
    if Csben(4)
      Ft(64*(6-1)+k,ck1) = 6; % MW
      slot_count(cPh) = slot_count(cPh) + 1;
      bandwidth(cPh) = bandwidth(cPh) + datawidth;
    end
    ...
    if (nPh==2 && Csben(127))          % positive product saved in Mdw (127)
      ...
      Ft(64*(8-1)+k,ck1) = 8; % MD
      slot_count(nPh) = slot_count(nPh) + 1;
      bandwidth(nPh) = bandwidth(nPh) + datawidth;
    end
...
%     end
    ...
    if (nPh==4)
      fprintf('\nclock=%6d\tbandwidth=%f,%f,%f\tslot=%6d,%6d,%6d\n', ck, bandwidth(2), bandwidth(3), bandwidth(4), slot_count(2), slot_count(3), slot_count(4));
  ...
  if (R(7,2)) 
    MAp(R(6,2)+1) = R(8,2); 
    Ft(516,ck1) = 11; % MB write
    slot_count(nPh) = slot_count(nPh) + 1;
    bandwidth(nPh) = bandwidth(nPh) + datawidth;
  end
  if (R(1,2)) 
    MX(R(2,2)+OP(ncp,6)+1,1:64) = R(3,2); 
    Ft(64*(3-1)+(1:64),ck1) = 3; % MB write
    slot_count(nPh) = slot_count(nPh) + 1;
    bandwidth(nPh) = bandwidth(nPh) + (datawidth+1)*64;
  end
  ...
  if mod(ck,200*DspRes)==0
    if mod(ck,400*DspRes)==0 
      view = [view(1:516,200*DspRes+1:1000*DspRes), Ft(1:516,200*DspRes+1:400*DspRes)];
      Ft(1:516,200*DspRes+1:400*DspRes) = 0;
    else
